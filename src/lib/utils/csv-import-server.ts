...
function mapRowToCustomerData(headers: string[], values: string[]): any {
  ...
  headers.forEach((header, index) => {
    ...
    if (fieldName) {
      ...
      // 請求書送付方法の変換
      if (fieldName === 'invoice_method' && value) {
        if (value === 'メール' || value === 'email') {
          value = 'email'
        } else if (value === '郵送' || value === 'mail') {
          value = 'mail'
        }
      }
...
// CSVテキストのパースとバリデーション（サーバーサイド用）
export async function parseCustomerCSVText(csvText: string): Promise<{
  data: any[]
  errors: ImportError[]
  headers: string[]
}> {
